            ...
            <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
                    ...
                    {navLinks.map((link, i) => {
                        ...
                        if (hasChildren) {
                            return (
                                <DropdownMenu.Root key={`${href || l.label}-${i}`}>
                                    <DropdownMenu.Trigger className="group flex items-center gap-1 text-sm font-medium text-slate-300 transition hover:text-white outline-none">
                                        {l.label}
                                    ...
                                    <DropdownMenu.Portal>
                                        <DropdownMenu.Content
                                            className="z-[60] min-w-[160px] rounded-xl border border-white/10 bg-slate-950 p-2 shadow-xl animate-in fade-in zoom-in duration-200"
                                            align="start"
                                            sideOffset={8}
                                        >
                        ...
                        return (
                            <Link
                                key={`${href}-${i}`}
                                href={href || "#"}
                                className="relative text-sm font-medium text-slate-300 transition hover:text-white group"
                            >
                                {l.label}
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 md:hidden">
                        ...
                        <button className="flex h-10 w-10 items-center justify-center rounded-lg border border-white/20 bg-white/5 p-2 text-slate-300 transition hover:bg-white/10">
