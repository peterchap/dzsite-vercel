    ...
    <section className={`relative overflow-hidden py-12 ${'bg-slate-950'}`}>
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          {intro ? <p className="mt-4 text-lg text-slate-300">{intro}</p> : null}
        </div>
        {/* Grid */}
        <div className="mt-12 grid gap-6 md:grid-cols-2">
          {groups?.map((g, idx) => (
            <div
              key={idx}
              className="group relative overflow-hidden rounded-3xl border border-white/10 bg-slate-950/80 p-6 shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:shadow-md"
            >
              {/* accent bar */}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-2 text-slate-300">{g.description}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        {/* Footer / ops callout */}
        {(footerLine || opsNote) && (
          <div className="mx-auto mt-12 max-w-4xl rounded-3xl border border-white/10 bg-slate-950/80 p-6 text-center shadow-sm backdrop-blur">
            {footerLine ? <p className="text-slate-300">{footerLine}</p> : null}
            {opsNote ? <p className="mt-2 text-sm text-slate-400">{opsNote}</p> : null}
          </div>
        )}
      </div>
    </section>
