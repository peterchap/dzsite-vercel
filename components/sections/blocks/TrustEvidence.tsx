...
export function TrustEvidence({
  ...
  return (
    <section id={anchor} className={cn("py-12 border-y border-white/10", "bg-slate-950")}>
      <Container>
        <div className="mx-auto max-w-7xl">
          <div className="overflow-hidden rounded-[2.5rem] border border-white/10 bg-slate-950/80 shadow-2xl px-8 py-12 md:px-16 md:py-16 text-center">
            {authorityStatement ? (
              <p className="mx-auto max-w-4xl text-xl md:text-2xl font-medium leading-relaxed text-white">
                {authorityStatement}
              </p>
            ) : null}
            {metrics?.length ? (
              <div className="mx-auto mt-12 grid max-w-5xl grid-cols-2 gap-6 md:grid-cols-4">
                {metrics.slice(0, 4).map((m, i) => {
                  ...
                  return (
                    <div key={i} className="rounded-2xl border border-white/10 bg-slate-950 p-6 text-center shadow-sm">
                      {m.icon && m.icon !== "none" ? (
                      ...
                      <div className="text-sm font-bold tracking-tight text-slate-300">{m.value}</div>
            ...
            {readinessSignals?.length ? (
              <div className="mx-auto mt-12 grid max-w-6xl grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5 auto-rows-fr justify-items-stretch items-stretch">
                {readinessSignals.slice(0, 6).map((s, i) => {
                  ...
                  return (
                    <div key={i} className="flex h-full w-full items-center justify-center gap-2 rounded-full border border-white/10 bg-slate-950 px-4 py-2 text-sm text-slate-300 shadow-sm">
            ...
            {socialProof ? (
              <p className="mx-auto mt-12 max-w-3xl text-sm text-slate-400">{socialProof}</p>
            ) : null}
          </div>
        </div>
      </Container>
