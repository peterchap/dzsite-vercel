        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-950 p-8 shadow-sm">
            <div className="text-center mb-6">
              ...
              <h3 className="mt-2 text-xl font-semibold text-white">{left.title}</h3>
            </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-950 p-8 shadow-sm">
            <div className="text-center mb-6">
              ...
              <h3 className="mt-2 text-xl font-semibold text-white">{right.title}</h3>
              {right.subtitle ? (
                <p className="mt-4 mx-auto max-w-md text-slate-300">{right.subtitle}</p>
              ) : null}
            </div>
            {right.featureSets && right.featureSets.length > 0 ? (
              <div className="space-y-8">
                {right.featureSets.map((set, i) => (
                  <div key={i} className="text-left">
                    {set.heading ? (
                      <h4 className="text-sm font-bold text-white uppercase tracking-wider mb-4 border-l-2 border-green-500 pl-3">
                        {set.heading}
                      </h4>
                    ) : null}
                    <ul className="space-y-3">
