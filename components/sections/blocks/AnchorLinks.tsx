...
export function AnchorLinks({ title, subtitle, selectedAnchors = [], isDark, behavior = "jump" }: any) {
  ...
  return (
    <section className={`relative overflow-hidden py-12 ${'bg-slate-950'} border-y border-white/10`}>
      <Container>
          ...
          {items.map((item: any, idx: number) => {
            ...
            return (
              <div key={idx} className="flex flex-col text-center h-full">
                {/* Feature Title */}
                {item.featureTitle && (
                  <h3 className="text-lg font-bold text-white mb-2">{item.featureTitle}</h3>
                )}
                {/* Feature Description */}
                {item.featureDescription && (
                  <div className="text-sm text-slate-300 mb-6 flex-1 px-4">
                ...
                <div className="mt-auto pt-2">
                  <ButtonLink
                    ...
                    className={`rounded-full px-8 py-2.5 text-sm font-semibold transition-all duration-300 shadow-sm w-full md:w-auto inline-block ${isActive
                      ...
                      : 'border-white/20 text-slate-300 hover:border-blue-400 hover:text-blue-600'
                      }`}
                  >
