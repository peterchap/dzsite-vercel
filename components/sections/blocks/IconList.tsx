    ...
    <section className={`py-12 ${"bg-slate-950"}`}>
      <Container>
        <div className="mx-auto max-w-3xl text-center">
          ...
          {description ? (
            <p className="mt-5 whitespace-pre-line text-lg text-slate-300">{description}</p>
          ) : null}
        </div>
            ...
            {items?.map((item, i) => {
              ...
              return (
                <li key={i} className="flex items-start gap-4">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-slate-950 shadow-sm">
                    {hasImage && imgUrl ? (
