            ...
            <Container>
                ...
                <div className="grid gap-8 lg:grid-cols-2">
                    {methods?.map((method, i) => {
                        ...
                        return (
                            <div key={i} className="flex flex-col h-full p-8 rounded-3xl border border-white/10 bg-slate-950 shadow-md">
                                <div className="mb-6 flex items-center gap-4">
                                    <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-50 text-blue-600">
                                        {IconComponent && <IconComponent className="h-6 w-6" />}
                                    </div>
                                    <h3 className="text-2xl font-bold text-white">{method.headline}</h3>
                                </div>
                                <div className="mb-8 flex flex-col gap-8">
                                    <div className="space-y-4">
                                        <h4 className="text-sm font-bold uppercase tracking-wider text-slate-400">Benefits</h4>
                                        <div className="text-slate-300 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:space-y-2 [&_ol]:list-decimal [&_ol]:pl-5 [&_ol]:space-y-2 [&_p]:mb-4">
                                ...
                                {method.codeSnippet && (
                                    <div className="mt-auto overflow-hidden rounded-xl bg-slate-950 p-4 text-xs font-mono text-slate-300">
                                        <div className="mb-2 flex items-center justify-between border-b border-slate-800 pb-2">
                                            <span className="text-slate-400">{method.codeSnippet.language}</span>
                                            <Icons.Copy className="h-3 w-3 text-slate-400 hover:text-slate-300 cursor-pointer" />
                                        </div>
                ...
                {diagram && (
                    <div className="mt-16 relative aspect-[16/6] w-full overflow-hidden rounded-3xl border border-white/10 bg-slate-950 shadow-xl">
                        <Image
