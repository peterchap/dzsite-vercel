    ...
    <section className={`relative overflow-hidden py-12 ${"bg-slate-950"}`}>
      <Container className="relative">
        <div className="max-w-3xl text-center mx-auto">
          {kicker && <p className="text-sm font-medium text-slate-400 text-center">{kicker}</p>}
          <h2 className="mt-2 text-3xl font-semibold tracking-tight text-white text-center">{title}</h2>
          {subtitle && <p className="mt-3 text-lg text-slate-300 text-center">{subtitle}</p>}
        </div>
        <div className={`mt-10 grid gap-x-6 gap-y-12 ${gridColsClass}`}>
          {cards?.map((c, idx) => (
            ...
            >
              ...
              <div className="absolute -inset-4 -z-10 rounded-3xl border border-white/10 bg-slate-950/80 shadow-sm backdrop-blur transition group-hover:-translate-y-0.5 group-hover:shadow-md" />
        ...
        {footer && (
          <div className="mt-10 max-w-3xl mx-auto text-center">
            <p className="text-lg text-slate-300">{footer}</p>
          </div>
        )}
