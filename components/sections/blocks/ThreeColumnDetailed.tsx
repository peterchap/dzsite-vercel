            ...
            <Container>
                ...
                <div className={`grid grid-cols-1 gap-8 ${columns.length === 2 ? "md:grid-cols-2 md:grid-rows-[auto_1fr_auto]" :
                    ...
                    {columns.map((col, i) => (
                        <div
                            key={i}
                            className="flex flex-col md:grid md:grid-rows-subgrid md:row-span-3 rounded-3xl border border-white/10 bg-slate-950 h-full shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md"
                        >
                            {/* 1) Title and Sub-title */}
                            <div className="p-6 border-b border-white/10 bg-white/5/50 flex flex-col justify-center">
                                {col.top?.title && (
                                    <h3 className="text-2xl font-bold tracking-tight mb-2 leading-tight text-white">
                                        {col.top.title}
                                    </h3>
                                )}
                                {col.top?.subtitle && (
                                ...
                                <ul className="space-y-3.5 mb-4">
                                    {col.middle?.features?.map((feat, fi) => (
                                        <li key={fi} className="flex items-start gap-4 group">
                                            <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-white/5 border border-white/10 text-sm shadow-sm group-hover:scale-110 transition-transform">
                                                {feat.iconEmoji ?? "•"}
                                            </div>
                                            <span className="text-sm text-slate-300 leading-normal font-medium">
                                                {feat.text}
                                            </span>
                                        </li>
                                    ))}
                            ...
                            <div className="p-6 border-t bg-blue-50/30 border-blue-100/50 flex flex-col">
                                ...
                                {col.bottom?.footer && (
                                    <div className="pt-6 border-t border-white/10/60 text-center">
                                ...
                                {col.cta && col.cta.label && (
                                    <div className="mt-8 pt-6 border-t border-white/10/60 flex justify-center">
                                        <ButtonLink
                                            href={col.cta.href || "#"}
