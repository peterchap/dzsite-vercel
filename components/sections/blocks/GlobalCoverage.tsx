  ...
  return (
    <section className={`py-12 overflow-hidden ${'bg-slate-950'}`}>
      <div className="container mx-auto px-4">
        ...
        {subtitle && (
          <p className="text-xl text-slate-300 text-center mb-24 max-w-3xl mx-auto">
            {subtitle}
          </p>
        )}
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            {points.map(
              (point: CoveragePoint, index: number) => (
                ...
                >
                  <div className={`bg-slate-950 rounded-xl shadow-lg p-4 flex flex-col items-center border border-gray-100/50 transition-transform hover:scale-105 ${point.position === 'bottom' ? 'min-w-[280px]' : 'min-w-[180px]'}`}>
                    <div className="text-sm text-white font-bold text-center line-clamp-2 leading-tight whitespace-nowrap">
                      {point.label}
                    </div>
                    <div className="text-xs text-slate-400 text-center mt-1.5 line-clamp-3 leading-normal">
                      {point.value}
                    </div>
                  </div>
                </div>
