            ...
            <div className="border-b border-white/10 bg-white/[0.03]">
                <div className="mx-auto max-w-6xl px-6 py-16 lg:py-20">
                    <div className="max-w-3xl">
                        <p className="text-sm font-bold uppercase tracking-widest text-blue-400">Trust & Safety</p>
                ...
                <div className="mx-auto max-w-3xl space-y-12">
                    ...
                    <LegalSection title="Reporting a Vulnerability">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                            ...
                            />
                            <div className="bg-white/[0.035] rounded-2xl p-6 border border-white/10 flex items-center">
                        ...
                        <div className="space-y-6">
                            <h4 className="font-bold text-white">Please include in your report:</h4>
                    ...
                    <LegalSection title="Our Commitment" border={false}>
                        ...
                        <div className="mt-12 p-8 rounded-3xl bg-white/[0.04] border border-white/10 text-white shadow-2xl relative overflow-hidden">
...
function ContactCard({ title, value, href, icon }: { title: string; value: string; href: string, icon: React.ReactNode }) {
    return (
        <a
            href={href}
            className="group p-6 rounded-2xl border border-white/10 bg-white/[0.035] shadow-sm hover:border-blue-400/40 transition-colors flex items-center gap-4"
        >
            ...
            <div>
                ...
                <p className="font-bold text-white group-hover:text-blue-400 transition-colors uppercase tracking-tight">{value}</p>
            </div>
        </a>
    );
}
...
function ReportItem({ text }: { text: string }) {
    return (
        <li className="flex items-start gap-3 p-4 rounded-xl border border-white/10 bg-white/[0.03]">
            <div className="mt-1 h-5 w-5 rounded-full bg-white/10 border border-white/20 flex items-center justify-center shrink-0">
                <div className="h-1.5 w-1.5 rounded-full bg-slate-300" />
            </div>
            <span className="text-sm font-semibold text-slate-300 leading-tight">{text}</span>
        </li>
    );
}
...
function GuidelineCard({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
    return (
        <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.035] flex gap-5 group hover:border-white/20 transition-colors">
...
function CommitmentItem({ title, text }: { title: string; text: string }) {
    return (
        <div className="flex items-baseline gap-4 py-4 border-b border-white/10 last:border-0">
            <span className="min-w-[180px] text-sm font-black text-white uppercase tracking-widest shrink-0">{title}</span>
